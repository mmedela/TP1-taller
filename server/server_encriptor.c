...
void fill_number_line(encriptor_t* self, char current_line[], int number_line[], size_t largo1, size_t largo2){
...
void multiplicar_matrices(encriptor_t* self, int num_line[]){
    int vec1[self->lado], vec2[self->lado], pos_num_line = 0, last_changed_pos = 0;
    ...
    while(!end){
        ...
        set_array_en_cero(self->lado, vec2);
        end = (pos_num_line == self->curr_num_line_len);
    }
}
